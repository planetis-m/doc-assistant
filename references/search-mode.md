...
Always build one raw semantic query string for `cvquery`.
...
Express filter behavior on the `cvquery` command line:
...
In `search` mode, infer filters from natural-language intent only when the user explicitly asks to narrow scope.

Semantic search is the default.

Treat these as explicit filtering cues:
...
- Pass filters through `cvquery` flags only when constrained retrieval is needed.
- Use only the filters the user actually needs.

## Outputting Results

- Evaluate the retrieved chunks against the user's exact query.
- Remove irrelevant or overly broad chunks that do not directly answer it.
- Rerank the remaining chunks so the most relevant ones appear first.
- Output the kept chunks verbatim and without raw metadata.
- Do not summarize, synthesize, rephrase, or interpret the retrieved text.
